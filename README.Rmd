...
# Chicago Bulls Roster Recomendations - Reproducible Data Analysis Project.

This repository contains data, Rscript, figures and report created to form a Moneyball style analysis using NBA data to pick a starting 5 for the Chicago Bulls. The task by the general manager of Chicago Bulls is to find the best five starting players (one from each position) within the teams budget of $118M. For the purpose of this task, we are focused on finding five starting players and can include players already on the Chicago Bulls roster.

## Getting Started

### Prerequisites

R Studio and install & load packages including.
...
You will need to load the data from the raw data files.

```
df_player <- read.csv("data/raw/2018-19_nba_player-statistics.csv")
```

## Defintition of variables
...
* PF : Personal Fouls
* PTS : Points
* NB: Players that were traded during the season may appear more than once (on more than one row) so it is important to handle these duplication appropriately.
...
##Run the tests through the coad chunks in the prject RMD file.
